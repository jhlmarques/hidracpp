...
Assembler::Assembler(Machine &_machine) :
    machine(_machine)
{
    assemblerMemory = QVector<Byte*>(machine.memory.size());
    reserved = QVector<bool>(machine.memory.size());
    PC = new Register("PC", 8); // TODO: Memory leak
}

void Assembler::clearAssemblerMemory()
{
    for (int i=0; i<assemblerMemory.size(); i++)
    {
        assemblerMemory[i]->setValue(0);
        reserved[i] = false;
        machine.correspondingLine[i] = -1;
    }
}

// Copies assemblerMemory to machine's memory
void Assembler::copyAssemblerMemoryToMachineMemory()
{
    for (int i=0; i<assemblerMemory.size(); i++)
        machine.memory[i]->setValue(assemblerMemory[i]->getValue());
}

// Reserve 'sizeToReserve' bytes starting from PC. Returns ErrorCode 0 if successful
Assembler::ErrorCode Assembler::reserveAssemblerMemory(int sizeToReserve)
{
...
bool Assembler::isValidValue(QString valueString, int min, int max)
{
    bool ok;
...
bool Assembler::isValidByteValue(QString valueString)
{
    return isValidValue(valueString, 0, 255);
}

/*bool Assembler::isValidRegisterValue(QString valueString)
{

}*/

bool Assembler::isValidAddress(QString addressString)
{
    return isValidValue(addressString, 0, assemblerMemory.size()-1);
}

bool Assembler::wasBuildSuccessful() const
{
    return buildSuccessful;
}

void Assembler::emitError(int lineNumber, Assembler::ErrorCode errorCode)
{
    QString errorString;
...
Assembler::ErrorCode Assembler::obeyDirective(QString mnemonic, QString arguments, bool reserveOnly)
{
    ...
    // TODO: dw, dab, daw (check endianness)
...
Assembler::ErrorCode Assembler::mountInstruction(QString mnemonic, QString arguments, QHash<QString, int> &labelPCMap)
{
    const Instruction *instruction = machine.getInstructionFromMnemonic(mnemonic);
    QStringList argumentList = arguments.split(" ", QString::SkipEmptyParts);
...
void Assembler::assemble(QString sourceCode)
{
    buildSuccessful = false;
    Assembler::ErrorCode errorCode;
    ...
    // TODO: Refactor labelPCMap as member
    QHash<QString, int> labelPCMap;
    clearAssemblerMemory(); // TODO: Review method
    PC->setValue(0);
    ...
    for (int lineNumber = 0; lineNumber < sourceLines.size(); lineNumber++)
    {
        if (!sourceLines[lineNumber].isEmpty())
        {
            ...
            if (instruction != NULL)
            {
                machine.correspondingLine[PC->getValue()] = lineNumber;
    ...
    copyAssemblerMemoryToMachineMemory();
